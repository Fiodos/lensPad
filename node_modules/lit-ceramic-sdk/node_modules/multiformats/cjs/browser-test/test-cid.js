...
describe('CID', () => {
  describe('v0', () => {
    ...
    test('create by parts', async () => {
      ...
      const cid$1 = cid.create(0, 112, hash);
      same(cid$1.code, 112);
      same(cid$1.version, 0);
    ...
    test('create from multihash', async () => {
      const hash = await sha2Browser.sha256.digest(buffer.Buffer.from('abc'));
      const cid$1 = cid.decode(hash.bytes);
      same(cid$1.code, 112);
      same(cid$1.version, 0);
    ...
    test('.bytes', async () => {
      const hash = await sha2Browser.sha256.digest(buffer.Buffer.from('abc'));
      const codec = 112;
      const cid$1 = cid.create(0, codec, hash);
    ...
    describe('decodeFirst', () => {
      test('no remainder', () => {
        const byts = bytes.fromHex('1220ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad');
        const [cid$1, remainder] = cid.decodeFirst(byts);
  ...
  describe('v1', () => {
    ...
    test('create by parts', async () => {
      const hash = await sha2Browser.sha256.digest(buffer.Buffer.from('abc'));
      const cid$1 = cid.create(1, 113, hash);
      same(cid$1.code, 113);
      same(cid$1.version, 1);
    ...
    test('.bytes', async () => {
      const hash = await sha2Browser.sha256.digest(buffer.Buffer.from('abc'));
      const code = 113;
      const cid$1 = cid.create(1, code, hash);
      const bytes$1 = cid$1.bytes;
      assert__default['default'].ok(bytes$1);
      const str = bytes.toHex(bytes$1);
  ...
  describe('utilities', () => {
    ...
    test('.isCid', () => {
      assert__default['default'].ok(cid.isCID(cid.parse(h1)));
      assert__default['default'].ok(!cid.isCID(false));
      assert__default['default'].ok(!cid.isCID(buffer.Buffer.from('hello world')));
      assert__default['default'].ok(cid.isCID(cid.parse(h1).toV0()));
      assert__default['default'].ok(cid.isCID(cid.parse(h1).toV1()));
    });
    test('works with deepEquals', () => {
      const ch1 = cid.parse(h1);
      ch1._baseCache.set('herp', 'derp');
      assert__default['default'].deepStrictEqual(ch1, cid.parse(h1));
      assert__default['default'].notDeepStrictEqual(ch1, cid.parse(h2));
    });
  });
  describe('throws on invalid inputs', () => {
    ...
    test('invalid fixtures', async () => {
      for (const test of invalidMultihash) {
        ...
        assert__default['default'].throws(() => cid.decode(buff), new RegExp(test.message));
      }
    });
  });
  ...
  describe('conversion v0 <-> v1', () => {
    test('should convert v0 to v1', async () => {
      ...
      const cid$1 = cid.create(0, 112, hash).toV1();
      same(cid$1.version, 1);
    });
    test('should convert v1 to v0', async () => {
      const hash = await sha2Browser.sha256.digest(buffer.Buffer.from(`TEST${ Date.now() }`));
      const cid$1 = cid.create(1, 112, hash).toV0();
      same(cid$1.version, 0);
    });
    ...
    test('should not convert v1 to v0 if not sha2-256 multihash', async () => {
      ...
      const cid$1 = cid.create(1, 112, hash);
  ...
  describe('caching', () => {
    ...
    test('should cache string representation when constructed with one', () => {
      const base32String = 'bafybeif2pall7dybz7vecqka3zo24irdwabwdi4wc55jznaq75q7eaavvu';
      const cid$1 = cid.parse(base32String);
      same(cid$1._baseCache.get(base32.base32.prefix), base32String);
    });
  });
  test('toJSON()', async () => {
  ...
  test('asCID', async () => {
    ...
    assert__default['default'].ok(cid.isCID(incompatibleCID));
    ...
    assert__default['default'].ok(cid1 instanceof cid);
    ...
    assert__default['default'].ok(cid3 instanceof cid);
    ...
    const cid5 = cid.asCID(new OLDCID__default['default'](1, 'raw', buffer.Buffer.from(hash.bytes)));
    assert__default['default'].ok(cid5 instanceof cid);
  ...
  describe('CID.parse', async () => {
    test('parse 32 encoded CIDv1', async () => {
      const hash = await sha2Browser.sha256.digest(buffer.Buffer.from('abc'));
      const cid$1 = cid.create(1, 112, hash);
      const parsed = cid.parse(cid$1.toString());
      digestsame(cid$1, parsed);
    });
    test('parse base58btc encoded CIDv1', async () => {
      const hash = await sha2Browser.sha256.digest(buffer.Buffer.from('abc'));
      const cid$1 = cid.create(1, 112, hash);
      const parsed = cid.parse(cid$1.toString(base58.base58btc));
      digestsame(cid$1, parsed);
    });
    ...
    test('fails to parse base64 encoded CIDv1', async () => {
      ...
      await testThrow(() => cid.parse(cid$1.toString(base64Browser.base64)), msg);
    });
    test('parses base64 encoded CIDv1 if base64 is provided', async () => {
      const hash = await sha2Browser.sha256.digest(buffer.Buffer.from('abc'));
      const cid$1 = cid.create(1, 112, hash);
      const parsed = cid.parse(cid$1.toString(base64Browser.base64), base64Browser.base64);
      digestsame(cid$1, parsed);
    });
  });
  test('inspect bytes', () => {
  ...
  test('new CID from old CID', async () => {
    const hash = await sha2Browser.sha256.digest(buffer.Buffer.from('abc'));
    const cid$1 = cid.asCID(new OLDCID__default['default'](1, 'raw', buffer.Buffer.from(hash.bytes)));
    same(cid$1.version, 1);
    assert__default['default'].ok(bytes.equals(cid$1.multihash, hash));
    same(cid$1.code, 85);
  });
